...
//			System.out.println("연결성공!!");
	...
	// 1-3. 페이징.
	public static List<Movie> pageing(List<Movie> allMovieList, int page) {
	...
	// 1-3-1. 모든 영화 목록 페이징해서 출력.
	public void showAllMovie(int page) {
		List<Movie> allMovieList = new ArrayList<>();
		allMovieList = getAllMovie();
		List<Movie> pagedMovieList = MovieDAO.pageing(allMovieList, page);
	...
	// 1-3-2. 예매 가능 영화 목록 페이징해서 출력.
	public void showAvailableMovie(int page) {
		List<Movie> availableMovieList = new ArrayList<>();
		availableMovieList = getAvailableMovie();
		List<Movie> pagedMovieList = MovieDAO.pageing(availableMovieList, page);
	...
	List<String[]> searchSchedule(String movieId) {
		...
		String sql = "SELECT  s.schedule_id, m.movie_name, "
				...
				+ "        s.discount, s.remain_seat " + "FROM    schedule s, movie m "
